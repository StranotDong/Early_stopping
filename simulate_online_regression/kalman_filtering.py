...
class oneIterPowerKalmanFilter:
    ...
    def __init__(self,
        ...
        # A = np.vstack([self.epochs, np.ones(len(self.x))]).T
        # a, b = np.linalg.lstsq(A, self.x, rcond=-1)[0]
        # self.s_est = a*self.epochs + b
        # self.d_est = np.mean(init_x - self.s_est)
