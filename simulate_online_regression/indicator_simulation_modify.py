...
"""
def early_stopping_prediction_adding_noise(a,b,c,d,linear_bias,var,min_delta,patience,
    ...
    y = a*np.power(x+c,b) + d*(x-linear_bias)
    ...
    for i in range(len(data)):
        ...
        if num_evals >= start_point and (num_evals-start_point)%period == 0:
            ...
            if not is_power_linear:
                ...
                var = np.var(original_data[s_noise_est:e]-(a*np.power(steps_noise_est+c,b)))
            else:
                ...
                c = res.x[3]
                d = res.x[2]**2
                var = np.var(original_data[s_noise_est:e]-(a*np.power(steps_noise_est+c,b) + d*(steps_noise_est-steps[0])))
            ...
            print("Predicted Stopping epoch is {}. a = {}, b = {}, c = {}, d^2={}".format(pred, a, b, c, d))
...
def mixKFIndicator(
    ...
    preds, coeffs, _, samples, smoothed_samples, CIs, _, predicted_steps = powerRegressionIndicator(
                                    ...
                                    is_fit_smoothed=False,
                                    noise_est_win_size = noise_est_win_size,
                                    bounds = ((0, None), (None, 0), (None,None)), inits = (1,0,0))
    rst_preds += preds
    rst_CIs += CIs
