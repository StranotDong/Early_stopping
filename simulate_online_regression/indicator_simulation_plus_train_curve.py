...
def powerRegressionIndicator(
                            ...
                            dynamic_patience_start=6000,
                            plateau_checkpoints=6
                            ):
    # original_data = data
    ...
    while True:
        ...
        if val_pointer >= on_val_start_point and (val_pointer - on_val_start_point) % period == 0:
            ...
            # # check plateau and change patience
            # if global_step >= dynamic_patience_start:
            #     s_plateau_check = len(shifts) - plateau_checkpoints
            #     step_thre = 0.5 * period * epochs_between_eval
            #     slope_thre = 0.5
            #     increase_decrease_patience_thre = (plateau_checkpoints) * period * epochs_between_eval
            #     # patience = patience_dynamic(shifts[s_plateau_check:], preds[s_plateau_check:], patience, check_plateau_thre, increase_decrease_patience_thre)
            #     patience, temp_flag = min_delta_dynamic(shifts[s_plateau_check:], preds[s_plateau_check:], patience, 
            #                                   -5, slope_thre, step_thre, increase_decrease_patience_thre,
            #                                   low_flag)
            #     low_flag = low_flag or temp_flag
            # print('Current patience is {} and min_delta is {}'.format(patience, min_delta))
...
def check_plateau(values, steps, slope_threshold, step_threshold, mode):
    ...
    # slope, intercept, r_value, p_value, std_err = scipy.stats.linregress(steps, values)


"""
change min_delta in the process of prediction
"""
def min_delta_dynamic(global_steps, preds, cur_min_delta, changing, slope_threshold, step_threshold, increase_decrease_patience_thre, prev_low_flag):
    ...
    # if any(i <= increase_decrease_patience_thre for i in remaining_time_list):
    # if remaining_time_list[-1] <= increase_decrease_patience_thre or remaining_time_list[len(remaining_time_list) - 2] <= increase_decrease_patience_thre:
        if not check_plateau(list(remaining_time_list), global_steps, slope_threshold, step_threshold, 'low'):
            return cur_min_delta, False
        # if round(cur_min_delta,4) > 5e-4:
        #     return max(cur_min_delta - 2*changing, 0), True
        return max(cur_min_delta - changing, 0), True

    # if the mean of the remaining time is larger than a threshold
    # and the curve is in the plateau
    # then make the patience - 1
    if prev_low_flag or not check_plateau(list(remaining_time_list), global_steps, slope_threshold, step_threshold, 'high') :
        return cur_min_delta, False
    # return min(cur_min_delta + changing, 1), False
    return cur_min_delta, False
