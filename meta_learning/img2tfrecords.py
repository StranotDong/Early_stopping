...
flags.DEFINE_string(
    'num_subsets', None, '# of total subsets')
flags.DEFINE_string(
    'subset_dir', None, 'Directory path for all the subset folders. '
    'Should have train and validation subdirectories inside each subset folder.')
FLAGS = flags.FLAGS
...
def _convert_to_example(filename, image_buffer, label, synset, height, width):
  ...
  # channels = 3
  # image_format = b'JPEG'
  colorspace = tf.compat.as_bytes('RGB')
  channels = 3
  image_format =tf.compat.as_bytes('JPEG')
  ...
  example = tf.train.Example(features=tf.train.Features(feature={
    ...
    # 'image/class/text': _bytes_feature(tf.compat.as_bytes('test')),
    # 'image/object/bbox/xmin':_int64_feature(10),
    # 'image/object/bbox/xmax':_int64_feature(20),
    # 'image/object/bbox/ymin':_int64_feature(10),
    # 'image/object/bbox/ymax':_int64_feature(20),
...
def convert_to_tf_records(raw_data_dir, local_scratch_dir):
  ...
  labels = {v: k for k, v in enumerate(
      sorted(set(validation_synsets + training_synsets)))}
...
def main(argv):
  for i in range(int(FLAGS.num_subsets)):
    print('{} out of {}'.format(i+1, FLAGS.num_subsets))
    raw_data_dir = FLAGS.subset_dir + '/' + str(i)
    # local_scratch_dir = FLAGS.subset_dir + '/' + str(i) + '_tfrecords'
    local_scratch_dir = './sub_datasets_tfrecords/' + str(i) + '_tfrecords'
    training_records, validation_records = convert_to_tf_records(raw_data_dir, local_scratch_dir)
