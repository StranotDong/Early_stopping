...
################################################################################
##########################
# my change: add a new argument: image_size
##########################
...
##########################
  ...
  dataset = dataset.apply(
      tf.contrib.data.map_and_batch(
          lambda value: parse_record_fn(value, is_training, dtype, image_size),
          batch_size=batch_size,
          num_parallel_batches=num_parallel_batches,
          drop_remainder=False))
  ############################
...
def learning_rate_with_decay(
  ...
  """
  ############################################
  # my change: make learning rate irrelevant to batch size
  ############################################
...
change -- add one more argument, train_size
"""
def resnet_main(
    flags_obj, model_function, input_function, dataset_name, train_size, shape=None):
#########################################################
  ...
  if len(schedule) == 0:
    ...
    for cycle_index, start_index in enumerate(schedule):
      ...
      classifier.train(input_fn=lambda: input_fn_train(start_index=start_index,
...
def define_resnet_flags(resnet_size_choices=None):
  ...
  """
  purpose -- val by steps not by epochs
  change -- add a flag: steps between eval
  """
  flags.DEFINE_integer(
            name="steps_between_evals", short_name="sbe", default=100,
            help=flags_core.help_wrap("The number of training steps to run between "
                                                  "evaluations."))
  #########################################################
